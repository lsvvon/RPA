        ...
        try:
            ...
            try:
                ...
                if "시세없음" in common_value_text:  # 시세없음인 경우 0 반환
                    common_numeric_value = 0
                    low_numeric_value = 0
                    print("일반가 값이 '시세없음'이므로 일반가와 하위평균가를 0으로 설정합니다.")
                else:
                    common_numeric_value = parse_cost_value(common_value_text)
                    # 하위평균가 가져오기
                    ...
                    low_numeric_value = parse_cost_value(low_value_text)
            except Exception as e:
                print(f"일반가 값을 가져오는 중 오류 발생. 오류: {e}")

            time.sleep(3)      

            print(f"하위평균가: {low_numeric_value}, 일반가: {common_numeric_value}")
            return low_numeric_value, common_numeric_value
