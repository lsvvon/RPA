...
def hometax_streetnum(driver, kwargs):
    ...
    try:
        ...
        try:
            # 오피스텔 및 상업용 건물 클릭
            office_building = WebDriverWait(driver, 10).until(
        ...
        try:
            # 법정동검색 클릭
            txppWframe_button = WebDriverWait(driver, 10).until(
        ...
        for row in rows:
            try:
                ...
                response["response_msg"] = f"해당 행의 선택 버튼 없음: {error}"
                response["data"] = [0, 0, 0, 0]
                return response
        ...
        try:
            ...
            # 검색버튼 클릭
            select_button2= WebDriverWait(driver, 10).until(
        ...
        try:
            # 모든 표시된 `txtItm` 요소 찾기
            visible_items = WebDriverWait(driver, 10).until(
            ...
            error = str(error).split("\n")[0]
            response["response_code"] = "90000001"
            response["response_msg"] = f"해당 물건지 찾기 도중 에러발생: {error}"
            response["data"] = [0, 0, 0, 0]
            return response
        
        # 상세주소 검색
        # 동
        select_dong = WebDriverWait(driver, 10).until(
        ...
        # Building_No1이 빈 값이면 첫 번째 옵션 선택
            ...
            try:
                ...
                error = str(error).split("\n")[0]
                response["response_code"] = "90000001"
                response["response_msg"] = f"{Building_No1} 값으로 선택하는 데 실패했습니다: {error}"
                response["data"] = [0, 0, 0, 0]
                return response
        ...
        try:
            ...
            time.sleep(2)
            # 호
            select_ho = WebDriverWait(driver, 10).until(
            ...
            error = str(error).split("\n")[0]
            response["response_code"] = "90000001"
            response["response_msg"] = f"층/호 선택하는 데 실패했습니다: {error}"
            response["data"] = [0, 0, 0, 0]
            return response

        try:
            # 검색버튼 클릭
            mf_txppWframe_btnSchTsv= WebDriverWait(driver, 10).until(
...
def hometax_roadnum(driver, kwargs):
    ...
    try:
        ...
        try:
            ...
            error = str(error).split("\n")[0]
            response["response_code"] = "90000001"
            response["response_msg"] = "해당 물건지 찾기 오류 발생." + error
            response["data"] = [0, 0, 0, 0]
            return response 
